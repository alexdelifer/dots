...
AddPackage ansible-lint # Checks playbooks for practices and behaviour that could potentially be improved.AddPackage aria2 # Download utility that supports HTTP(S), FTP, BitTorrent, and Metalink
...
AddPackage babel-cli # A compiler for writing next generation JavaScript
...
AddPackage cowsay # Configurable talking cow (and a bit more)
AddPackage curlie # The power of curl, the ease of use of httpie.
AddPackage davfs2 # File system driver that allows you to mount a WebDAV folder
...
AddPackage espflash # Cargo subcommand for flashing Espressif devices over serial
...
AddPackage glab # Cli tool to help work seamlessly with GitLab from the command line
...
AddPackage mingw-w64-tools # MinGW-w64 utilities
...
AddPackage mqttui # Subscribe to a MQTT Topic or publish something quickly from the terminal
...
AddPackage scc # Sloc, Cloc and Code: a very fast accurate code counter with complexity calculations and COCOMO estimates written in pure Go
...
AddPackage upx # Extendable, high-performance executable packer for several executable formats
